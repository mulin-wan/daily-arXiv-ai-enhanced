...
### [1] [Ontology-Constrained Neural Reasoning in Enterprise Agentic Systems: A Neurosymbolic Architecture for Domain-Grounded AI Agents](https://arxiv.org/abs/2604.00555)
*Thanh Luong Tuan*

Main category: cs.AI

TL;DR: This paper presents a neurosymbolic architecture within the FAOS platform that mitigates LLM limitations through ontology-constrained neural reasoning, significantly enhancing enterprise agent performance across multiple industries.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenges of hallucination, domain drift, and regulatory compliance in LLM adoption for enterprises.

Method: A three-layer ontological framework (Role, Domain, Interaction) is introduced to constrain inputs and outputs in LLM-based agent reasoning. The approach includes a controlled experiment across diverse industries to evaluate performance metrics.

Result: Ontology-coupled agents outperformed ungrounded agents significantly on various metrics: Metric Accuracy, Regulatory Compliance, and Role Consistency, with greatest improvements in less represented domains.

Conclusion: The study provides a formal ontology model, neurosymbolic coupling taxonomy, and empirical validation, contributing to enhanced LLM applications in enterprises.
...
TL;DR: The paper addresses the issue of fragmented indexing in structured documents by proposing a method to construct semantically complete document chunks called Evidence Units (EUs).


<details>
  <summary>Details</summary>
Motivation: The need to improve retrieval effectiveness by addressing the fragmentation of semantically cohesive document elements in indexing processes.

Method: A pipeline is introduced to create EUs through ontology-grounded role normalization, a semantic global assignment algorithm, a graph-based decision layer in Neo4j, and cross-parser validation.

Result: Experiments showed significant improvements in retrieval effectiveness, with an increase in LCS and Recall@1, indicating better configuration of document chunks.

Conclusion: The proposed EU-based chunking method effectively enhances the retrieval of structured documents, retaining improvements across different parsers.
